...
| **KYB is two legs, not one submit** | `/kyc/legal-person` is not "PF with another body". Upstream there is still no PJ *provider*: the partner-api orchestrates KYB as (1) a **synchronous** company check against the Receita registry and (2) the **legal representative's** documentoscopy - the ordinary PF flow over their CPF. So a `201` means *the company passed, the person still has to verify*. Three refusals to handle, all before any webview: `400 KYB_REPRESENTATIVE_MISSING` (an owner with `ownerType` `LEGAL_REPRESENTATIVE`/`BOTH` is **required** - the form disables submit without one), `422 KYB_COMPANY_REJECTED` (**no session is persisted**, so the result arrives on `useKyc.companyRejection`, never on `session`), `503 KYB_REGISTRY_UNAVAILABLE` (fail-closed, retry). This closed BUG-H18. See `docs/04-partner-api.md` -> *KYB*. |
